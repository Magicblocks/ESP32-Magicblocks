...
public:
    ...
    unsigned int getData(String& data, uint8_t *opcode = NULL);
    unsigned int getData(char* data, uint8_t *opcode = NULL);
    // Write data to the stream
    void sendData(const char *str, uint8_t opcode = WS_OPCODE_TEXT);
    void sendData(unsigned char *str,int size, uint8_t opcode = WS_OPCODE_TEXT);
    void sendData(String str, uint8_t opcode = WS_OPCODE_TEXT);
    void printGPRS(String s);
    String readResponse(int steps);
    void reset();
    char *path;
    char *host;
    char *protocol;
...
private:
    ...
    unsigned int handleStream(String& data, uint8_t *opcode);    
     unsigned int handleStream(char* data, uint8_t *opcode);
    ...
    void sendEncodedData(unsigned char *str,int size, uint8_t opcode);
